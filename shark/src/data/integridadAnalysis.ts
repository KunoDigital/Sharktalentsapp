...
export function analizarPerfilIntegridad(dims: DimensionResult[]): {
  ...
  if (buenaImpresion) {
    if (buenaImpresion.nivel === 'alto') {
      ...
      validezNota = 'Hubo cierta tendencia a responder lo socialmente esperado. Considera el reporte como un piso (los riesgos no son menores a lo que muestra, pero podrían ser un poco mayores).';
    }
  }
...
const PREGUNTAS_POR_DIMENSION: Record<string, string[]> = {
  honestidad: [
    'Cuéntame de una situación en el trabajo anterior donde tuviste que reconocer un error que afectó al equipo. ¿Cómo lo manejaste?',
    'Si un compañero te pide que cubras una falta o un error, ¿qué haces?',
  ],
  confiabilidad: [
    'En tu trabajo anterior, ¿qué porcentaje de las veces llegabas a tiempo? ¿Por qué crees que era así?',
    'Cuéntame de un compromiso que asumiste y no pudiste cumplir. ¿Qué pasó?',
  ],
  dominio_personal: [
    'Cuéntame de un conflicto reciente con un cliente o compañero. ¿Cómo reaccionaste?',
    '¿Cómo respondes cuando alguien te critica injustamente delante de otros?',
  ],
  autenticidad: [
    '¿Hay cosas que dices frente a tu jefe que no dirías frente a tus compañeros? ¿Por qué?',
  ],
  imparcialidad: [
    '¿Cómo tomas decisiones cuando tienes que elegir entre dos compañeros para un proyecto? ¿Influyen tus simpatías personales?',
  ],
  sencillez: [
    'Cuéntame de un logro reciente del que estés orgulloso. ¿Cómo lo comunicaste al equipo?',
  ],
  inteligencia_social: [
    '¿Te pasó alguna vez decir algo en el trabajo que resultó inoportuno? Cuéntame.',
  ],
  alcohol: [
    '¿Cómo manejas situaciones laborales que involucran alcohol (cierres de venta, eventos, after-office)?',
    'Valida con referencias: ¿hubo episodios de impuntualidad o ausentismo de lunes?',
  ],
  drogas: [
    'Valida con referencias laborales recientes y considera test toxicológico si el rol lo justifica.',
  ],
  apuestas: [
    '¿El rol incluye manejo de caja, dinero o información financiera? Si sí, valida con referencias y considera no asignar acceso a fondos hasta tener más historial.',
  ],
  hurto: [
    'Si el rol incluye acceso a inventario, productos o efectivo, valida referencias laborales específicas sobre esa función.',
    'Considera controles periódicos de inventario los primeros meses.',
  ],
  soborno: [
    '¿El rol incluye decisiones sobre proveedores, compras o atención a clientes que dan regalos? Si sí, refuerza la política de cumplimiento desde el día 1.',
  ],
  buena_impresion: [
    'Profundiza con preguntas situacionales concretas en lugar de hipotéticas — pídele ejemplos puntuales de su trabajo anterior con nombres, fechas y resultados verificables.',
  ],
};
